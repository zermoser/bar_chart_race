...
const PopulationChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5001/api/getPopulation/viewFilter');
        setAllData(transformData(response.data.population));
      } catch (error) {
    ...
    function updateData() {
      ...
      if (allData[currentYear]) {
        ...
        am5.array.each(series.dataItems, (dataItem) => {
          ...
          const value = allData[currentYear][category];
          if (value > 0) {
            itemsWithNonZero++;
          }
          dataItem.animate({
